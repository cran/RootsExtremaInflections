inflexi <-
  ...
  #Function to Search for all available inflection-points p=x_i
  i=NULL;
  #
  fs=function(i,x1,y1){
    pr1<-x1[i];
    ...
    v0n=as.double(abs(c1$coeff[3]))
    points=cbind(v0n,pr1)
    names(points)=c("a2","x0")
  ...
  colnames(mans)=c("a2","x0")
  rownames(mans)=1:dim(mans)[1]
  #Find minimum value |a_2| and corresponding root x_i=p
  n0<-which.min(mans$a2);
  ipc<-mans[n0,"x0"];
  ...
  if(plots){
  if(plotpdf){pdf('inflectionplot.pdf')}
  par(mfrow=c(1,2))
  ymin<-min(y1);
  ymax<-max(y1);
  dy<-(ymax-ymin)/5;
  ...
  if(plotpdf){cat("File 'inflectionplot.pdf' has been created","\n");dev.off()}
  #Return output...
